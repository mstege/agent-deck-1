...
- `session send` no longer reports success for a message that never arrived. The default path treated a bare `active` status as proof of its own submission, but an agent that was ALREADY working looks identical whether or not the keystrokes landed — so "active" is evidence only as a change from before the send. `verifyContentArrival` states exactly that rule for its own path ("an agent that was already busy stays busy regardless, so that case proves nothing"); the default path did not apply it. Observed 2026-09-09 on `conductor-stayplace`: three consecutive sends each reported `✓ Sent message`, none arrived, and the composer was left holding `1. Yes1. Yes1. Yes` — one unsubmitted copy per "successful" send. A pre-send baseline is now taken on both paths, and a busy target is settled by the delivery receipt instead of by its own busyness.
- The delivery receipt is consulted on the content-arrival path too, not only in the submit-verification loop. A send that took that path could still end in the `#1793` verdict — "the body is visible but the agent never began processing it" — while the agent's own hook had already recorded the prompt. Two codepaths reach that verdict and fixing one left the other.
- No delivery-failure message recommends resending any more. All three ended with advice to do it — `#876` with "before retrying", both `#1793` variants with "Treat this as NOT delivered — the submitting Enter may have been swallowed" — and on 2026-09-09 all three kinds of wrong verdict were observed in one day: success reported where nothing arrived, and failure reported where everything arrived (a completed `/clear` under `#876`, a message being processed under `#1793`). A verdict that is wrong in both directions must not carry an instruction, least of all the one that duplicates a delivered message; for a deletion order, following it is unrecoverable. The two verdicts that mean "we could not confirm" now say `UNCONFIRMED` and warn explicitly against a blind resend, and `#876` no longer reports as "message not delivered" but as "delivery is unconfirmed".
- A dead-lettered event no longer appears in every conductor's drain. `inbox drain` counted the whole dead-letter directory to avoid reporting a clean state, so one parked record with no target at all — a child created `--no-parent`, dead-lettered as `child_removed` — warned and exited non-zero for five unrelated conductors, each of which had to dismiss it and escalate while its heartbeat drain looked broken. A drain now surfaces only the records addressed to that session; records naming no target are reported as a note that does not fail the drain, and `agent-deck inbox dead-letters [--json]` is the read-only operator surface that lists them grouped by the session each failed to reach.
- `session send` no longer reports a completed `/clear` as a silent drop. A session-resetting slash command destroys every pane-based proof of its own delivery by succeeding — no `active` transition, no composer remnant, and its own line erased — so the verification loop burned its whole budget and then recommended a resend of a command that had already run. A replaced agent session id is the evidence that survives, and it now counts as a receipt for exactly those commands. Measured on the reproduced case: the delivery phase went from 16.2s and `no_evidence` to 413ms and `submitted`.
- The submit verification loop is bounded in wall-clock time, not only in iterations. Each of its 50 checks makes two tmux subprocess calls capped at 3s apiece, so the nominal ~15s loop became minutes on a loaded machine — five consecutive `session send` calls exceeded a 120s caller timeout on 2026-09-09. The loop now stops at 60s and reports the checks it actually ran rather than the cap it was given.
- `session send --json` reports where its wall clock went, as `phase_ms` per phase (defer hold, readiness wait, delivery, completion wait). A caller that hit a timeout could previously not tell a long hold on a busy target from a slow readiness wait from a slow delivery — three problems with three different fixes.
- An unrecognised command is no longer answered with the TUI. The dispatch switch had no default, so any unknown token fell through to the interactive launch: outside a session that opens a full-screen UI in answer to a typo, and inside one it printed "Cannot launch the agent-deck TUI inside an agent-deck session", naming a cause unrelated to the mistake. `agent-deck account` reproduces it in one word. Unknown commands now report as such, honour `--json`, and name the intended command when exactly one is unambiguous.
- An unknown `session` subcommand honours `--json` and names the command that does the job. `agent-deck session list --json` printed human usage text on stdout, so `json.load` failed with "Expecting value: line 1 column 1" — indistinguishable from an empty result. It now says the listing lives one level up as `agent-deck list --json`.
- `session children --json` carries `model`, `path`, `branch`, `tool`, `substate` and `account`. It reported only id, title and status, so building a fleet overview meant one `session show` per child — N extra process spawns for fields this listing already had loaded.
- A Claude session mid-turn is no longer reported as `waiting`. The hook handler maps three different Claude events to that one status — `Stop` (the turn ended), `PermissionRequest` and `Notification` with a permission-prompt matcher (something mid-turn needs you) — and nothing downstream could tell them apart. Under `--dangerously-skip-permissions` the last two mean nobody is waiting at all, so a session went on generating while a fresh `waiting` record stood for the full hook freshness window and the transition daemon notified its parent. The pane body settles it: Claude prints a live status line and a `ctrl+c to interrupt` hint while, and only while, a turn is in flight, and a real permission prompt stops both — so a turn in flight now keeps the session marked as running. It reuses the busy patterns this repo already ships and the same pane capture the background-work check makes, and the promotion is one-way, so a failed capture leaves the verdict exactly as it was. Not the pane title, which is the obvious candidate and no longer carries the signal: 4484 title samples across 118 panes on Claude Code v2.1.266 contained zero Braille frames and only the done marker, on working sessions included — which also means `GetStatus`'s title fast path no longer fires for Claude.
- Commands that act on "the current session" (`session output`, `session attach`, `mcp attached`, `skill attached`, `worktree`, `handoff`) no longer fail with a token nobody typed. A tmux session is named `agentdeck_<title>_<suffix>` where the suffix is four random bytes from `generateShortID()`; `GetCurrentSessionID` read it as the instance id, so `agent-deck session output` inside `agentdeck_ad-zustellung_b18d8430` reported `session 'b18d8430' not found` while the instance id was `f2e1578b-1788958022`. Resolution now prefers `AGENTDECK_INSTANCE_ID` and otherwise defers to the title-based parser that `session show` already used correctly.
- A session that cannot be resolved now says what would have worked. Every other outcome of `ResolveSession` lists its candidates; not-found — the most common one — listed nothing, so a typo, a rename and a wrong profile all looked the same. It now suggests similar titles with their ids (bounded), or reports how many sessions are registered and points at `agent-deck list`.
